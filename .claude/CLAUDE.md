...
This is a Claude Code plugin marketplace providing the **project-tracker** plugin — 5 skills (init, learn, doctor, update, adr) for structured project documentation in `.claude/project-tracker/`. Each skill is a SKILL.md file; skills share shell scripts in `scripts/` and document templates in `templates/`.
...
#   plugins/project-tracker/.claude-plugin/plugin.json  →  "version"
```
...
- `.claude-plugin/marketplace.json` — entry point, sources plugin from `./plugins/project-tracker`
- `plugins/project-tracker/` — one plugin with 5 skills
...
**Data flow**: Skills write docs to `.claude/project-tracker/` in the user's workspace (not this repo). This repo's own `.claude/project-tracker/` is the self-test — it was generated by running `/project-tracker:init` in this repo.
